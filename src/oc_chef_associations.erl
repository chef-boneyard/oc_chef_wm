...
-spec deprovision_removed_user(#base_state{}, #chef_user{}, oc_authz_id()) -> deprovision_response().
deprovision_removed_user(State, User, RequestorAuthzId) ->
    ...
    deprovision_removed_user0(Result, Context).

deprovision_removed_user0({ok, USAG}, #context{ db_context = DbContext, org_id = OrgId } = Context) ->
    Result = chef_db:fetch(#oc_chef_group{org_id = OrgId, name = "users"}, DbContext),
    deprovision_removed_user1(Result, Context#context{usag = USAG});
deprovision_removed_user0(Error, _Context) ->
    {error, {error_fetching_usag,Error}}.

deprovision_removed_user1({ok, OrgUsersGroup}, #context{usag = USAG,
    ...
    deprovision_removed_user2(Result, Context#context{org_users = OrgUsersGroup1});
deprovision_removed_user1(Error, _Context) ->
    {error, {error_fetching_org_users_group, Error}}.

deprovision_removed_user2({ok, _}, #context{usag = USAG, db_context = DbContext } = Context) ->
    % TODO this usag entity will still exist in bifrost
    Result = chef_db:delete_object(USAG, DbContext),
    deprovision_removed_user3(Result, Context);
deprovision_removed_user2(Error, _Context) ->
    {error, {error_removing_from_org_user_group, Error}}.

deprovision_removed_user3({ok, _}, #context{authz_context = AuthzContext,
                                            org_name = OrgName} = Context) ->
    Result = oc_chef_authz_db:fetch_global_group_authz_id(AuthzContext, OrgName, "admins"),
    deprovision_removed_user4(Result, Context) ;
deprovision_removed_user3(Error, Context) ->
    ...
    deprovision_removed_user3({ok, ok}, Context = #context{msg = [{usag_record_delete_failed, Error}]}).

deprovision_removed_user4({ok, OrgGlobalAdminsAuthzId}, #context{ user_authz_id = UserAuthzId} = Context) ->
...
deprovision_removed_user4(Error, #context{msg = Msg}) ->
...
provision_associated_user(State, #chef_user{id = UserId} = User, RequestorAuthzId) ->
    ...
    provision_associated_user0(Result, Context#context{usag = USAG0}).

provision_associated_user0({ok, AuthzId}, #context{usag = USAG,
    ...
    provision_associated_user1(Result, Context#context{usag = USAG0});
provision_associated_user0(Error, _Context) ->
...
provision_associated_user1(ok, #context{usag = USAG,
                                        user_name = UserName,
                                        requestor_authz_id = RequestorAuthzId,
                                        db_context = DbContext} = Context) ->
    USAG0 = USAG#oc_chef_group{users = [UserName]},
    Result = chef_db:update(USAG0, DbContext, RequestorAuthzId),
    provision_associated_user2(Result, Context#context{usag = USAG0});
provision_associated_user1(Error, _Context) ->
    {error, {usag_creation_failed, Error}}.

provision_associated_user2(ok, #context{usag = USAG,
                                        org_id = OrgId,
                                        requestor_authz_id = RequestorAuthzId,
                                        db_context = DbContext} = Context) ->
    ...
    provision_associated_user3(Result, Context);
provision_associated_user2(Error, _Context) ->
    {error, {add_user_to_usag_failed, Error}}.

provision_associated_user3(ok, #context{org_name = OrgName,
                                        authz_context = AuthzContext } = Context) ->
    Result = oc_chef_authz_db:fetch_global_group_authz_id(AuthzContext, OrgName, "admins"),
    provision_associated_user4(Result, Context);
provision_associated_user3(Error, _Context) ->
    {error, {add_usag_to_org_users_group_failed, Error}}.

provision_associated_user4({ok, OrgGlobalAdminsAuthzId}, #context{user_authz_id = UserAuthzId } = Context) ->
...
provision_associated_user4(Error, _) ->
